...
def match_text(comp_path,params):
    global ele
    global max_score
    ele = None
    max_score = -1
    ...
    if ele is not None:
        # special
        if ele.attrib['text'].lower() in ['travel articles','travel communities']:
            time.sleep(8)
            print('special sleep 8 seconds')
...
def image_to_words_comp(img,lang='eng',params=None):
    ...
    img_tw0 = img_gray.point(lambda x: 255 if x > params['txt_binary_threshold'] else 0)
    if lang == 'eng':
