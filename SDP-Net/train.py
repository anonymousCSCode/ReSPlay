...
from libs.AdbCommand import checkIfInstalled,screencap,dump_layout,call_adb
...
def train_apk(pkName,activityName,params):
    ...
    for trace in trace_list:
        ...
        checkIfInstalled(pkName,activityName)
        restart_flag = False
        trace_flag = False
        step_index = 0
            ...
            while True:
                ...
                if restart_flag:
                    for item in range(step_index):
                        ...
                        exec_action(action_item,params)
                        time.sleep(2)
                    restart_flag = False
                print('current step index, ',step_index)
                # current state start
                screencap(CACHE_FRONT)
                ...
                if text_flag:
                    ...
                    random_index = match_text(component_path,params)
                    text_flag = False
                    if random_index == -2:
                ...
                exec_action(random_index,params=params)
                time.sleep(8)
